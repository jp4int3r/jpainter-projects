  ...
  if ($_SERVER['REQUEST_METHOD'] ==
    'POST') {
     ...
     if (!$problem) { 
        print '<p class="text-
          -success">You are now
          registered!<br>Okay, you are not
          really registered but...</p>';
		  
		  $body = "Thank you, {$_POST['first_name']}, for 
          registering with the J.D. 
          Salinger fan club!'.";
          mail($_POST['email'], 'Registration Confirmation', 
          $body, 'From: jp4int3r@gmail.com');
...
?>
	<form action="register.php" method="post" class="form--inline">
        ...
        size="20" value="<?php if (isset($_POST['last_name'])) { print htmlspecialchars(
        $_POST['last_name']); } ?>"></p>

		<p><label for="email">Email Address:</label><input type="email" name="email" 
        size="20" value="<?php if (isset($_POST['email'])) { print htmlspecialchars(
        $_POST['email']); } ?>"></p>
        ...
        name="password2" size="20" value="<?php if(isset($_POST['password2'])) { print
        htmlspecialchars($_POST['password2']); } ?>"></p>
